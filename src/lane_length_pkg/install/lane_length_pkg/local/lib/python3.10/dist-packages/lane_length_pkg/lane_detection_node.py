...
class LaneDetectionNode(Node):
    def __init__(self):
        ...
        try:
            ...
            config_dir = '/home/alexlee/fsd_ws/src/lane_length_pkg/config'
        ...
        self.declare_parameter('publish_rectified_image', False)

        # Lane extraction params
        self.declare_parameter('lane_roi_top_ratio', 0.58)
        self.declare_parameter('lane_roi_bottom_ratio', 0.90)
        ...
        self.declare_parameter('max_lane_miss', 10)
        self.declare_parameter('right_offset_ratio', 0.30)
        self.declare_parameter('right_line_left_offset_m', 0.10)
        self.declare_parameter('left_line_right_offset_m', 0.10)
        self.declare_parameter('follow_mode', 'center')
        self.declare_parameter('target_y_ratio', 0.98)  # 실행 명령 호환용. metric 모드에서는 steer_forward_m 기준 사용
        ...
        self.declare_parameter('src_top_y_ratio', 0.60)
        ...
        self.declare_parameter('forward_query_margin_m', 0.0)
        ...
        self.declare_parameter('status_requires_current_detection', False)
        self.declare_parameter('use_farthest_visible_width_for_stop', True)

        # overlay params
        self.declare_parameter('draw_metric_overlay', True)
        self.declare_parameter('debug_view_bottom_crop_ratio', 0.10)
        # show_debug_view=True일 때 4분할 화면의 오른쪽 위(result) 영역 왼쪽에 표시
        ...
        self.publish_rectified_image = bool(self.get_parameter('publish_rectified_image').value)
        ...
        self.right_line_left_offset_m = float(
            self.get_parameter('right_line_left_offset_m').value)
        self.left_line_right_offset_m = float(
            self.get_parameter('left_line_right_offset_m').value)
        self.follow_mode = str(self.get_parameter('follow_mode').value).strip().lower()
        if self.follow_mode not in ('center', 'left', 'right'):
        ...
        self.forward_query_margin_m = float(self.get_parameter('forward_query_margin_m').value)
        ...
        self.use_farthest_visible_width_for_stop = bool(self.get_parameter('use_farthest_visible_width_for_stop').value)
        self.draw_metric_overlay = bool(self.get_parameter('draw_metric_overlay').value)
        self.debug_view_bottom_crop_ratio = max(
            0.0,
            min(0.90, float(self.get_parameter('debug_view_bottom_crop_ratio').value)),
        )
        ...
        self.warned_invalid_dist_coeff_sources = set()
        ...
        self.guidance_source_pub = self.create_publisher(String, '/lane_guidance_source', 10)
        ...
        self.left_error_m_pub = self.create_publisher(Float32, '/lane_error_left_m', 10)
        ...
        self.debug_left_steer_m_pub = self.create_publisher(Float32, '/lane_debug/left_steer_m', 10)
        self.debug_right_steer_m_pub = self.create_publisher(Float32, '/lane_debug/right_steer_m', 10)
        self.debug_center_raw_m_pub = self.create_publisher(Float32, '/lane_debug/center_raw_m', 10)
        self.debug_zero_bias_m_pub = self.create_publisher(Float32, '/lane_debug/lateral_zero_bias_m', 10)
        ...
        if self.publish_bev_image:
            self.bev_image_pub = self.create_publisher(Image, '/lane_bev_image', 10)
        else:
            self.bev_image_pub = None
        if self.publish_rectified_image:
            self.rectified_image_pub = self.create_publisher(Image, '/image_rectified', 10)
        else:
            self.rectified_image_pub = None
        ...
        self.get_logger().info(
            f'detection_node started | topic={self.image_topic} fps={self.process_fps:.1f} '
    ...
    def image_callback(self, msg: Image):
        try:
            ...
            if self.rectified_image_pub is not None:
                self.rectified_image_pub.publish(self.bridge.cv2_to_imgmsg(rectified, encoding='bgr8'))
    ...
    def normalize_dist_coeffs(self, d: np.ndarray, model: str, source: str) -> Optional[np.ndarray]:
        coeffs = np.asarray(d, dtype=np.float64).reshape(-1)
        valid_lengths = (4, 5, 8, 12, 14)
        if coeffs.size not in valid_lengths:
            warn_key = (str(source), int(coeffs.size))
            if warn_key not in self.warned_invalid_dist_coeff_sources:
                self.warned_invalid_dist_coeff_sources.add(warn_key)
                self.get_logger().warning(
                    f'ignoring camera calibration from {source}: '
                    f'distortion coefficient count {coeffs.size} is not one of {valid_lengths}'
                )
            return None
    ...
    def set_camera_calibration(self, k: np.ndarray, d: np.ndarray, model: str, source: str) -> None:
        model = str(model).strip().lower()
        dist_coeffs = self.normalize_dist_coeffs(d, model, source)
    ...
    def rectify_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self.rectify_shape != (w, h) or self.rectify_map1 is None or self.rectify_map2 is None:
            ...
            try:
                ...
                self.get_logger().warning(f'camera rectification setup failed; using raw frame: {e}')
                self.rectify_map1 = None
                self.rectify_map2 = None
                self.rectify_shape = None
                return frame
        ...
        try:
            ...
            self.get_logger().warning(f'camera rectification failed; using raw frame: {e}')
            self.rectify_map1 = None
            self.rectify_map2 = None
            self.rectify_shape = None
            return frame
    ...
    def build_metric_bev_image(self, frame: np.ndarray) -> Optional[np.ndarray]:
        if self.M_metric is None:
            return None
    ...
    def relative_metric_error_from_image_point(self, img_pt_xy: np.ndarray) -> Optional[float]:
        """
        Old working controller convention:
        do not use absolute BEV lateral as control error because H.npy/BEV center
        is not guaranteed to be base_link lateral zero.

        Instead compare the target image point against the image-center point at
        the same detection-warp y, then convert that pair to metric BEV and use
        their BEV-x difference. This makes the control error a true relative
        left/right error around the robot image center.
        """
        ...
        ego_warped_pt = np.array(
            [self.width * 0.5, warped_pt[1]], dtype=np.float32
        ).reshape(1, 1, 2)
        ego_img_pt = cv2.perspectiveTransform(ego_warped_pt, self.Minv_detect).reshape(2)

        bev_pts = self.image_points_to_metric_bev(
            np.stack([img_pt.reshape(2), ego_img_pt], axis=0)
        )
    ...
    def sample_lane_base(self):
        ...
        base_result = {
            ...
            'stop_query_m': self.stop_forward_m,
            'center_lateral_m': None,
            'left_target_lateral_m': None,
        ...
        stop_query_m = self.stop_forward_m
        if self.use_farthest_visible_width_for_stop:
            stop_query_m = min(
                self.stop_forward_m,
                float(left_curve[0][-1]),
                float(right_curve[0][-1]),
            )
            stop_query_m = max(stop_query_m, self.visible_bottom_base_m)

        left_stop = self.sample_lateral_at_forward(left_curve, stop_query_m)
        right_stop = self.sample_lateral_at_forward(right_curve, stop_query_m)
        ...
        left_target_lateral_m = float(
            left_steer + max(0.0, self.left_line_right_offset_m)
        )
        left_target_lateral_m = min(
            center_lateral_m,
            max(float(left_steer), left_target_lateral_m),
        )
        if self.right_line_left_offset_m > 0.0:
            right_target_lateral_m = float(
                right_steer - max(0.0, self.right_line_left_offset_m)
            )
            right_target_lateral_m = max(
                center_lateral_m,
                min(float(right_steer), right_target_lateral_m),
            )
        else:
            right_target_lateral_m = float(
                center_lateral_m + self.right_offset_ratio * lane_width_steer_m
            )

        # Absolute lateral values are still kept for visualization/path publishing.
        # Control error is computed later from image-relative metric error, exactly
        # like the old working node.
        center_error_m = 0.0
        left_error_m = 0.0
        right_error_m = 0.0
        if self.follow_mode == 'left':
            target_lateral_m = left_target_lateral_m
        ...
        left_target_img_pt = self.base_points_to_image(
            np.array([left_target_lateral_m], dtype=np.float32),
            np.array([self.steer_forward_m], dtype=np.float32),
        )
        ...
        left_target_img_pt = left_target_img_pt[0] if left_target_img_pt is not None and left_target_img_pt.shape[0] == 1 else None
        ...
        center_error_valid = False
        left_error_valid = False
        right_error_valid = False
        if center_img_pt is not None:
            center_rel_m = self.relative_metric_error_from_image_point(center_img_pt)
            if center_rel_m is not None and math.isfinite(float(center_rel_m)):
                center_error_m = float(center_rel_m)
                center_error_valid = True
        if left_target_img_pt is not None:
            left_rel_m = self.relative_metric_error_from_image_point(left_target_img_pt)
            if left_rel_m is not None and math.isfinite(float(left_rel_m)):
                left_error_m = float(left_rel_m)
                left_error_valid = True
        if right_target_img_pt is not None:
            right_rel_m = self.relative_metric_error_from_image_point(right_target_img_pt)
            if right_rel_m is not None and math.isfinite(float(right_rel_m)):
                right_error_m = float(right_rel_m)
                right_error_valid = True

        if self.follow_mode == 'left':
            follow_error_m = float(left_error_m)
            follow_error_valid = left_error_valid
        elif self.follow_mode == 'right':
            follow_error_m = float(right_error_m)
            follow_error_valid = right_error_valid
        else:
            follow_error_m = float(center_error_m)
            follow_error_valid = center_error_valid
    ...
    def publish_center_base_point(self, center_lateral_m: Optional[float], lane_ok: bool):
        ...
        if lane_ok and center_lateral_m is not None and math.isfinite(float(center_lateral_m)):
    ...
    def publish_visible_base_paths(self, lane: dict):
        ...
        self.left_boundary_base_path_pub.publish(self.build_path_msg(forward, left_lat))
        self.right_boundary_base_path_pub.publish(self.build_path_msg(forward, right_lat))
    ...
    def crop_debug_view_bottom(self, img: np.ndarray) -> np.ndarray:
        if self.debug_view_bottom_crop_ratio <= 0.0:
            return img
        keep_h = max(1, int(round(img.shape[0] * (1.0 - self.debug_view_bottom_crop_ratio))))
        return img[:keep_h, ...]

    def build_debug_view(self, original: np.ndarray, result: np.ndarray, binary: np.ndarray, warped: np.ndarray):
        original = self.crop_debug_view_bottom(original)
        result = self.crop_debug_view_bottom(result)
        binary = self.crop_debug_view_bottom(binary)
        warped = self.crop_debug_view_bottom(warped)
    ...
    def draw_metric_text_overlay(
        ...
        """
        예전 화면처럼 4분할 중 오른쪽 위(result) 화면 왼쪽에
        px/m/rad 값을 흰 글씨로 직접 표시한다.
        """
        ...
        lines = [
            ...
            f'stop_width_m: {self.fmt3(lane_width_m)}',
            f'stop_x_m: {self.fmt3(stop_query_m)}',
            f'target_x_m: {self.fmt3(steer_query_m)}',
    ...
    def process_frame(self):
        ...
        if current_lane_ok and lane['target_img_pt'] is not None:
            ...
            left_pt_img = np.round(lane['left_target_img_pt']).astype(np.int32) if lane['left_target_img_pt'] is not None else None
            ...
            if left_pt_img is not None:
                left_error_px = float(left_pt_img[0] - image_center_x)
                cv2.circle(result, tuple(left_pt_img), 6, (180, 255, 180), -1)
        ...
        stop_query_m = float(lane.get('stop_query_m', self.stop_forward_m)) if current_lane_ok else self.stop_forward_m
        steer_query_m = float(self.steer_forward_m)
        center_error_m = float(lane['center_error_m']) if current_lane_ok else 0.0
        left_error_m = float(lane['left_error_m']) if current_lane_ok else 0.0
        ...
        if current_lane_ok and None not in (lane['left_stop_m'], lane['right_stop_m']):
            stop_img = self.base_points_to_image(
                ...
                np.array([stop_query_m, stop_query_m], dtype=np.float32),
            )
        ...
        self.publish_scalar(self.left_error_m_pub, left_error_m)
        self.publish_scalar(self.right_error_m_pub, right_error_m)
        self.publish_scalar(self.follow_error_m_pub, follow_error_m)
        left_steer_m = float(lane['left_steer_m']) if current_lane_ok and lane['left_steer_m'] is not None else 0.0
        right_steer_m = float(lane['right_steer_m']) if current_lane_ok and lane['right_steer_m'] is not None else 0.0
        self.publish_scalar(self.debug_left_steer_m_pub, left_steer_m)
        self.publish_scalar(self.debug_right_steer_m_pub, right_steer_m)
        self.publish_scalar(self.debug_center_raw_m_pub, 0.5 * (left_steer_m + right_steer_m))
        self.publish_scalar(self.debug_zero_bias_m_pub, self.lateral_zero_bias_m)
        self.publish_scalar(self.stop_forward_m_pub, float(lane.get('stop_query_m', self.stop_forward_m)))
        ...
        source = String()
        source.data = 'live' if current_lane_ok else 'none'
        self.guidance_source_pub.publish(source)
        ...
        if self.bev_image_pub is not None:
            bev_img = self.build_metric_bev_image(frame)
            if bev_img is not None:
                self.bev_image_pub.publish(self.bridge.cv2_to_imgmsg(bev_img, encoding='bgr8'))

        if self.show:
            if self.show_debug_view:
                debug_view = self.build_debug_view(frame, result, binary, warped)
                # 4분할 debug_view 기준:
                # 왼쪽 위=원본, 오른쪽 위=result이므로
                # result 영역의 왼쪽에 오버레이를 그리기 위해 frame.shape[1]만큼 x offset 적용
