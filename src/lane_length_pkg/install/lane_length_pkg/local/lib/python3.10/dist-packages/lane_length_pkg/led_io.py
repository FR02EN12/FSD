...
class LedIoNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('arduino_blocked_ports', ['/dev/tb3_lidar'])
        ...
        self.declare_parameter('arduino_flush_after_write', False)
        self.declare_parameter('arduino_drain_input', True)
        self.declare_parameter('arduino_repeat_sec', 1.0)
        ...
        self.arduino_blocked_ports = [
            str(port) for port in self.get_parameter('arduino_blocked_ports').value
        ]
        ...
        self.arduino_flush_after_write = bool(
            self.get_parameter('arduino_flush_after_write').value)
        self.arduino_drain_input = bool(
            self.get_parameter('arduino_drain_input').value)
        self.arduino_repeat_sec = max(
            0.0, float(self.get_parameter('arduino_repeat_sec').value))
        ...
        self.last_arduino_payload: Optional[bytes] = None
        self.last_arduino_write_time: float = 0.0
    ...
    def _candidate_arduino_ports(self) -> list[str]:
        ...
        for port in ports:
            ...
            if not port or port in result:
                continue
            if self._is_blocked_arduino_port(port):
                self.get_logger().warn(
                    f'Skipping blocked Arduino LED serial port: {port}')
                continue
            result.append(port)
        return result

    def _is_blocked_arduino_port(self, port: str) -> bool:
        for blocked in self.arduino_blocked_ports:
            blocked = blocked.strip()
            if not blocked:
                continue
            if port == blocked:
                return True
            if os.path.exists(port) and os.path.exists(blocked):
                try:
                    if os.path.realpath(port) == os.path.realpath(blocked):
                        return True
                except OSError:
                    pass
        return False
    ...
    def _set_arduino_mask(self, mask: str) -> None:
        ...
        now = time.monotonic()
        if (
            payload_bytes == self.last_arduino_payload
            and (now - self.last_arduino_write_time) < self.arduino_repeat_sec
        ):
            return

        try:
            self.last_arduino_payload = payload_bytes
            self.last_arduino_write_time = now
            if self.arduino_drain_input:
                self.serial_conn.reset_input_buffer()
            self.serial_conn.write(payload_bytes)
            if self.arduino_flush_after_write:
