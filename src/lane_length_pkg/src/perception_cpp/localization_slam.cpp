// localization_slam.cpp -- EKF-based localization with optional occupancy grid.
// Fuses /scan + /odom + /imu, publishes /pose and /tf by default.
#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstring>
...
public:
  LocalizationSlamNode() : Node("localization_slam") {
    ...
    declare_parameter<bool>("publish_map_odom_tf", true);
    declare_parameter<bool>("identity_map_odom_tf", false);
    declare_parameter<bool>("publish_map", false);
    ...
    // Map parameters
    declare_parameter<double>("map_resolution", 0.025);
    declare_parameter<int>("map_width", 160);
    declare_parameter<int>("map_height", 160);
    declare_parameter<double>("map_origin_x", -2.0);
    declare_parameter<double>("map_origin_y", -2.0);
    ...
    declare_parameter<bool>("clear_map_each_update", false);
    ...
    declare_parameter<double>("scan_yaw_offset_deg", 0.0);
    declare_parameter<double>("scan_ignore_angle_min_deg", -30.0);
    declare_parameter<double>("scan_ignore_angle_max_deg", 30.0);
    declare_parameter<double>("scan_ignore_range_max_m", 0.35);
    declare_parameter<bool>("scan_ignore_all_ranges", false);
    ...
    identity_map_odom_tf_ = get_parameter("identity_map_odom_tf").as_bool();
    ...
    clear_map_each_update_ = get_parameter("clear_map_each_update").as_bool();
    ...
    use_scan_matching_ = get_parameter("use_scan_matching").as_bool();
    scan_yaw_offset_rad_ = get_parameter("scan_yaw_offset_deg").as_double() * M_PI / 180.0;
    scan_ignore_min_rad_ = get_parameter("scan_ignore_angle_min_deg").as_double() * M_PI / 180.0;
    scan_ignore_max_rad_ = get_parameter("scan_ignore_angle_max_deg").as_double() * M_PI / 180.0;
    scan_ignore_range_max_m_ = get_parameter("scan_ignore_range_max_m").as_double();
    scan_ignore_all_ranges_ = get_parameter("scan_ignore_all_ranges").as_bool();
...
private:
  ...
  bool identity_map_odom_tf_{false};
  ...
  bool clear_map_each_update_{false};
  double sm_range_;
  int sm_skip_;
  bool use_scan_matching_{false};
  double scan_yaw_offset_rad_{0.0};
  double scan_ignore_min_rad_{0.0};
  double scan_ignore_max_rad_{0.0};
  double scan_ignore_range_max_m_{0.0};
  bool scan_ignore_all_ranges_{false};
  ...
  void scan_cb(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    last_scan_ = msg;
    has_scan_ = msg && !msg->ranges.empty();
  }
  ...
  bool scan_in_ignore_sector(double angle, double range) const {
    const double a = normalize_angle(angle);
    const double lo = normalize_angle(scan_ignore_min_rad_);
    const double hi = normalize_angle(scan_ignore_max_rad_);
    const bool in_angle = (lo <= hi) ? (lo <= a && a <= hi) : (a >= lo || a <= hi);
    if (!in_angle) return false;
    if (scan_ignore_all_ranges_) return true;
    return std::isfinite(range) && range <= scan_ignore_range_max_m_;
  }
  ...
  Correction scan_match() {
    ...
    for (int i = 0; i < n; i += sm_skip_) {
      ...
      double angle = normalize_angle(scan->angle_min + i * scan->angle_increment + scan_yaw_offset_rad_);
      if (scan_in_ignore_sector(angle, r)) continue;
  ...
  void update_occupancy_grid() {
    ...
    for (int i = 0; i < n; i += sm_skip_) {
      ...
      double local_angle = normalize_angle(scan->angle_min + i * scan->angle_increment + scan_yaw_offset_rad_);
      if (scan_in_ignore_sector(local_angle, r)) continue;
      ...
      double angle = local_angle + theta_;
  ...
  // Publish TF: map -> odom
  void publish_tf() {
    if (!publish_map_odom_tf_) return;
    if (!has_odom_) return;

    // map->odom = map->base * inv(odom->base)
    double tf_theta = identity_map_odom_tf_ ? 0.0 : normalize_angle(theta_ - odom_theta_);
    double cos_t = std::cos(tf_theta), sin_t = std::sin(tf_theta);
    double tf_x = identity_map_odom_tf_ ? 0.0 : x_ - (cos_t * odom_x_ - sin_t * odom_y_);
    double tf_y = identity_map_odom_tf_ ? 0.0 : y_ - (sin_t * odom_x_ + cos_t * odom_y_);
  ...
  void publish_map() {
    if (!publish_map_ || !pub_map_) return;
    if (!has_scan_) return;
